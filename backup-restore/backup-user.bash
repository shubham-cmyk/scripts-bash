...
read -pr "Enter redis cluster name [$DEFAULT_CLUSTER_NAME]: " CLUSTER_NAME
CLUSTER_NAME=${CLUSTER_NAME:-$DEFAULT_CLUSTER_NAME}

read -pr "Enter redis cluster namespace [$DEFAULT_CLUSTER_NAMESPACE]: " CLUSTER_NAMESPACE
CLUSTER_NAMESPACE=${CLUSTER_NAMESPACE:-$DEFAULT_CLUSTER_NAMESPACE}

read -pr "Enter Redis port [$DEFAULT_REDIS_PORT]: " REDIS_PORT
REDIS_PORT=${REDIS_PORT:-$DEFAULT_REDIS_PORT}

read -pr "Enter Redis password [$DEFAULT_REDIS_PASSWORD]: " REDIS_PASSWORD
...
read -pr "Enter your choice (1, 2, or 3): " BACKUP_DESTINATION

case "$BACKUP_DESTINATION" in
    1)
        read -pr "Enter AWS S3 bucket name : " AWS_S3_BUCKET
        read -pr "Enter AWS S3 bucket region : " AWS_DEFAULT_REGION
        read -pr "Enter AWS access key : " AWS_ACCESS_KEY_ID
        read -pr "Enter AWS secret access key : " AWS_SECRET_ACCESS_KEY
        RESTIC_REPOSITORY="s3:s3.${AWS_DEFAULT_REGION}.amazonaws.com/${AWS_S3_BUCKET}/${CLUSTER_NAME}-${CLUSTER_NAMESPACE}"
        ;;
    2)
        read -pr "Enter Azure Blob Storage container name : " AZURE_CONTAINER
        read -pr "Enter Azure storage account name : " AZURE_ACCOUNT_NAME
        read -pr "Enter Azure storage account key : " AZURE_ACCOUNT_KEY
        RESTIC_REPOSITORY="azure:${AZURE_CONTAINER}:${CLUSTER_NAME}-${CLUSTER_NAMESPACE}"
        ;;
    3)
        read -pr "Enter Google Cloud Storage bucket name: " GCP_BUCKET
        read -pr "Enter Google Cloud Project ID: " GCP_PROJECT_ID
        read -pr "Enter path to Google Cloud key file: " GCP_KEY_FILE
